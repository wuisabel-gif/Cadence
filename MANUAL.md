...
- **Other coding agents** — the shared skill also has install targets for Kimi
  Code CLI, ZCode Agent, standalone Claude Code, OpenCode, and Grok Build. These
  use the model already configured in the host. Grok Bot has no drop-in folder;
  see [integrations/grok-bot/README.md](integrations/grok-bot/README.md). See
  [integrations/agents/README.md](integrations/agents/README.md).
...
seminar, dispatch, column, kin, essence. Once it loads, skip the slash command and just ask —
...
| `essence` | First-principles argument: strip a domain to its laws, reason up, land on conviction. |
...
| `npm run install:agent -- --agent <host>` | Install for `kimi`, `zcode`, `claude-code`, `opencode`, `grok`, or `codex`. |
